...
def generar_mapa_calor(ruta_relativa, data, file):
    ...
    plt.savefig(f"{ruta_relativa}/grafiques/{file.split('.')[0]}_sense_norm.png")  # Guardar el mapa de calor en un archivo (eliminant l'extensió)

def recorrer_directori(ruta, ruta_relativa):
    ...
    for file in os.listdir(ruta):
        if file.endswith(".txt") or (file.endswith(".out")):
            with open(os.path.join(ruta, file), "r") as f:
                df = preparar_dades(f, normalitzar=False)

                generar_mapa_calor(ruta_relativa, df, file)

if __name__ == "__main__":
    ruta_relativa = "M2"
    ruta = os.path.join(os.getcwd(), ruta_relativa)
    recorrer_directori(ruta, ruta_relativa)