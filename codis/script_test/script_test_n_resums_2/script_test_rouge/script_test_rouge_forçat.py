"""
    Este script s'encarregara de comparar amb ROUGE-Lsum els resums generats forçant la font amb els resums de referència
    I generaran una matriu per poder comparar amb els resultats de script_test_rouge_per_font.py per poder comparar si han millorat
    o empitjorat els resultats forçant la font
"""
...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", eixida = "resultats_script_test_fonts_ampliat.out", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca"):
    ...
    def evaluate_model(subset = "test_i"):
        ...
        for token_font in tqdm(tokens_fonts, desc="Columnes:", position=0): #este bucle governa les columnes (font forçada)
            ...
            resums = json.load(open(f"./resums_generats/resums_{subset}_forçat_{token_font}.json", "r"))
            ...
            for tkn_font in tqdm(tokens, desc="Files:", position=1): #este bucle governa les files (font original)
                ...
                for batch in tqdm(DataLoader(resums_per_font[str(tkn_font)], batch_size=32, shuffle=False), desc="Batch:", position = 2):
    ...
    print("Matriu_i:")
    print(matriu_i)
    with open("./resultats_rouge/rouge_forçat_i.txt", "w") as f:
        escriure_matriu(matriu_i, f)
        escriure_matriu(matriu_i, stdout)

    matriu_ni = evaluate_model(subset="test_ni")
    print("Matriu_ni")
    print(matriu_ni)
    with open("./resultats_rouge/rouge_forçat_ni.txt", "w") as f:
        escriure_matriu(matriu_ni, f)
        escriure_matriu(matriu_ni, stdout)
