...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", eixida = "resultats_script_test_font_correcta.out", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca"):
    ...
    """
    def evaluate_model(model, test_dataset, avaluar_classif = False):
        model.eval()
        ...
        for token_font in tokens_fonts:
            ...
            for batch in tqdm(DataLoader(test_dataset[str(token_font)], collate_fn = data_collator, batch_size=8, shuffle=True), desc="Avaluant el model"):
                ...
                #avaluem sí o sí el format i la correcció per veure que hagem fet correctament el codi
        ...
        #estes dos mètriques les deixem sí o sí per si ens haguérem enganyat en el codi
        resultat["classificacio_accuracy"] = total_encerts / total_mostres
        resultat["classificacio_format_correcte"] = total_format_correcte / total_mostres
    ...
    res_i, matriu_i = evaluate_model(model, tokenized_test_i, avaluar_classif = False)
