...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", eixida = "resultats_script_test_fonts_ampliat.out", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca", num_token = 0):
    nltk.download('punkt')
    ...
    def evaluate_model(subset = "test_i"):
        ...
        #com volem "paralel·litzar-ho" ho llancem en diferents processos per a cada font original
        ...
        for token_font in tqdm(tokens_fonts, desc="Columnes:", position=0): #este bucle governa les columnes (font forçada)
            ...
            for id, resum in resums_forçats.items():
                ...
                #com ara anem a generar una única fila, només volem les notícies que siguen de la font que s'ha mirat
                if src in tokens: 
    ...
    if num_token < len(tokens_fonts): evaluate_model(subset="test_i")
    if num_token < len(tokens_fonts_ni): evaluate_model(subset="test_ni")
...
if __name__ == "__main__":
    ...
    if tkns == None:
        ...
        exit()
    if tkns < 0 or tkns > 6:
        print("El paràmetre ha de ser un número entre 0 i 5. Per a català. Perquè només hi ha 6 fonts")
        exit()
    print(tkns)
    main(num_token = tkns)