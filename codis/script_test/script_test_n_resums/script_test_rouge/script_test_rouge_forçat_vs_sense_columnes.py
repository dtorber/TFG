...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", eixida = "resultats_script_test_fonts_ampliat.out", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca", tokens_forçats = None):
    nltk.download('punkt')
    ...
    if tokens_forçats is None:
        tokens_forçats = tokens_fonts
    ...
    def evaluate_model(subset = "test_i"):
        ...
        for tkn_font in tokens:
            ...
            for token_font in tokens_forçats:
        ...
        for token_font in tqdm(tokens_forçats, desc="Columnes:", position=0): #este bucle governa les columnes (font forçada)
        ...
        #nomes generem els casos en els que forcem la font, no tenim tanta informació per fer també per font original
        for token_font in tokens_forçats:
            ...
            with open(f"./resultats_rouge/per_font_forçada/resums_diferents_{subset}_forçat_{str(token_font)}.txt", "w") as fitxer:
                if (len(resums_forçats_per_font[str(tkn_font)]) * len(tokens_forçats)) != 0:
                    fitxer.write(f"Total casos diferents: {len(noticies_diferents)} / {len(resums_forçats_per_font[str(tkn_font)]) * len(tokens_forçats)} ({round(len(noticies_diferents) / (len(resums_forçats_per_font[str(tkn_font)]) * len(tokens_forçats)), 4)}%)\n\n")
...
if __name__ == "__main__":
    #llegir el primer paràmetre que s'ha rebut
    tkns = int(sys.argv[1]) if len(sys.argv) > 1 else None
    if tkns == None:
        print("No s'ha passat cap paràmetre. CAL ESPECIFICAR LA FONT A FORÇAR")
        exit()
    if tkns < 60002 or tkns > 60007:
        print("El paràmetre ha de ser un número entre 60002 i 60007. Per a català")
        exit()
    print(tkns)
    main(tokens_forçats = [tkns])