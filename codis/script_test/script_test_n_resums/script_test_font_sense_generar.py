...
def main(checkpoint="NASca-finetuned-de-zero-amb-metriques-anonim", carregar_de_hugginface=False, carregar_de_cache=True, llengua="ca"):
    ...
    def evaluate_model(subset = "test_i", tkn_font = None):
        model.eval()
        resums_json = json.load(open(f"../resums_generats/n_resums/resums_{subset}_forçat_{tkn_font}.json", "r"))
    ...
    for tkn in tokens:
        f = open("../resultats_n_resums/resultats_fonts/resultats_script_test_font_" + str(tkn) + ".json", "w")
