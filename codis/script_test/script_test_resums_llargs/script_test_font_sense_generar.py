...
def main(checkpoint="NASca-finetuned-de-zero-amb-metriques-anonim", carregar_de_hugginface=False, carregar_de_cache=True, llengua="ca"):
    ...
    #com ara els resums són més llargs s'han de comparar amb els resums de la font correcta:
    test_i = json.load(open("../resums_generats/resums_llargs/resums_test_i_font_correcta.json","r"))
    ...
    def evaluate_model(subset = "test_i", tkn_font = None):
        ...
        resums_json = json.load(open(f"../resums_generats/resums_llargs/resums_{subset}_forçat_{tkn_font}.json", "r"))
    ...
    for tkn in tokens:        
        f = open("../resultats_resums_llargs/resultats_fonts/resultats_script_test_font_" + str(tkn) + ".out", "w")
        ...
        res_i, matriu_i = evaluate_model(subset = "test_i", tkn_font = tkn)
