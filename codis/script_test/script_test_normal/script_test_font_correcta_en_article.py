...
test_i_combinat = test_i.map(lambda x: {
    "article": f"{x['source']}{SEP_TOKEN}{x['article']}{tokenizer.eos_token}", #article en principi el deixem igual que estava
...
def evaluate_model(model, test_dataset, avaluar_classif = False, tkn_font = None):
    ...
    for batch in tqdm(DataLoader(test_dataset, collate_fn = data_collator, batch_size=8, shuffle=True), desc="Avaluant el model"):
        ...
        summary_ids = []
        max_len = 0
        for input_id, label in zip(input_ids, resum_referencia):
            resum = model.generate(
                torch.unsqueeze(input_id, 0), num_beams=4, min_length=25, max_length=150,
                ...
                #forced_bos_token_id = label[0], #forcem a que comence com si fora de la font correcta
            )
            #ho passem a llista perquè si gastem np o torch fa falta que tinguen les mateixes dimensions i precisament encara no tenim això
            resum_llista = resum.tolist()[0]
            summary_ids.append(resum_llista)
            if len(resum_llista) > max_len:
                max_len = len(resum_llista)

        #Ara hem de fer un padding perquè totes les prediccions tinguin la mateixa longitud
        for i in range(len(summary_ids)):
            summary_ids[i] = summary_ids[i] + [tokenizer.pad_token_id] * (max_len - len(summary_ids[i]))
        
        #I ja ho passem a tensor perquè el decode està esperant un tensor bidimensional
        summary_ids = torch.cuda.IntTensor(summary_ids)
...
f = open("resultats_script_test_font_correcta.out", "w")
