...
for particio in corpus_ca.keys():
    resums_catala = []
    for i in range(len(corpus_ca[particio])):
        entrada = dict()
        entrada["lang"] = "ca"
        entrada["summary_ca"] = corpus_ca[particio][i]["summary"]
        entrada["source"] = corpus_ca[particio][i]["source"]
        entrada["article"] = corpus_ca[particio][i]["article"] 
        ...
        resums_catala.append(entrada["summary_ca"])
    traduccions_catala = translate_text(resums_catala, "ca", "es")
    for i in range(len(corpus_ca_es[particio])):
        corpus_ca_es[particio][i]["summary_es"] = traduccions_catala[i]
...
for particio in corpus_es_aux.keys():
    resums_espanyol = []
    for i in range(len(corpus_es_aux[particio])):
        ...
        if source in equivalencia and conteig_es[particio].get(source, 0) < conteig_ca[particio].get(equivalencia[source], 0):
            ...
            article = corpus_es_aux[particio][i]["article"]

            entrada = dict()
            entrada["summary_es"] = summary
            entrada["article"] = article
            entrada["id"] = id
            entrada["source"] = source
            entrada["lang"] = "es"
    ...
    for i in range(len(corpus_es[particio])):
        corpus_es[particio][i]["summary_ca"] = traduccions_espanyol[i]
...
for particio in corpus_ca_es.keys():
    with gzip.open(f"../corpus_anonimitzat/ca-es2/{particio}.json.gz", "wt") as f:
