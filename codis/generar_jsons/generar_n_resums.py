...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", carpeta = "./resums_generats", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca"):
    ...
    #ha de ser <= num_beams per tant fem 4 que és lo màxim permès
    num_resums_a_generar = 4
    ...
    """
    def generate_summarys(model, test_dataset, test_original, ruta = f"./resums_generats/n_resums/{datetime.now()}.json", forced_token = None, subset = "test_i"):        
        i = 0
        json_data = dict()
        for batch, batch_original in zip(tqdm(DataLoader(test_dataset, collate_fn = data_collator, batch_size=8, shuffle=False), desc="Generant resums"), DataLoader(test_original, batch_size=8, shuffle=False)):
            ...
            summary_ids = model.generate(
                ...
                num_return_sequences = num_resums_a_generar,
            )
            ...
            while i < len(predictions):
                aux.append(predictions[i:i+num_resums_a_generar])
                i += num_resums_a_generar
    ...
    """
        ...
        @return el json que hem creat per si se vol aprofitar per alguna cosa més que només escriure-ho en un fitxer
    """
    def generate_summary_font_correcta(model, dataset_original, ruta = f"./resums_generats/n_resums/{datetime.now()}.json"):        
        i = 0
        json_data = {}
    ...
    generate_summarys(model, tokenized_test_i, test_i, ruta = f"{carpeta}/n_resums/resums_test_i_original.json", subset = "test_i", )
    generate_summarys(model, tokenized_test_ni, test_ni, ruta = f"{carpeta}/n_resums/resums_test_ni_original.json", subset = "test_ni")
    for token in tokens_fonts:
        generate_summarys(model, tokenized_test_i, test_i, ruta = f"{carpeta}/n_resums/resums_test_i_forçat_{token}.json", forced_token = token, subset = "test_i")
        generate_summarys(model, tokenized_test_ni, test_ni,  ruta = f"{carpeta}/n_resums/resums_test_ni_forçat_{token}.json", forced_token = token, subset = "test_ni")
    ...
    generate_summary_font_correcta(model, test_i, ruta = f"{carpeta}/n_resums/resums_test_i_font_correcta.json")
