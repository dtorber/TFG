...
def main(checkpoint= "NASca-finetuned-de-zero-amb-metriques-anonim", carpeta = "./resums_generats", eixida = "resultats_script_test.out", carregar_de_hugginface = False, carregar_de_cache = True, llengua = "ca"):
    nltk.download('punkt')
    ...
    """
    def generate_summarys(model, test_dataset, test_original, carpeta = "./resums_generats", ruta = f"./resums_generats/resums_llargs/{datetime.now()}.json", forced_token = None, subset = "test_i"):        
        i = 0
        json_data = dict()
        ...
        for batch, batch_original in zip(tqdm(DataLoader(test_dataset, collate_fn = data_collator, batch_size=8, shuffle=False), desc="Generant resums"), DataLoader(test_original, batch_size=8, shuffle=False)):
            ...
            summary_ids = model.generate(
                input_ids, num_beams=4, min_length=70, max_length=420,
    ...
    def generate_summary_font_correcta(model, dataset_original, ruta = f"./resums_generats/{datetime.now()}.json"):        
        ...
        for token_font in tokens_fonts:
            ...
            for batch in tqdm(DataLoader(test_dataset[str(token_font)], collate_fn = data_collator, batch_size=8, shuffle=False), desc="Generant resums"):
                ...
                #ara se força la longitud dels resums perquè siga mínim el doble de la mitjana de longitud: 35 * 2 i mantenim la propoció de x6 per al màxim
                summary_ids = model.generate(
                    input_ids, num_beams=4, min_length=70, max_length=420,
                    early_stopping=True, no_repeat_ngram_size=3, repetition_penalty=2.0,
                    #decoder_start_token_id=1, #forcem a que comence amb el token 1 i així no dona problemes lo del 2,1,1,1, però en realitat sí que dona perquè alguns són 1,1,1
    ...
    generate_summarys(model, tokenized_test_i, test_i, ruta = f"{carpeta}/resums_llargs/resums_test_i_original.json", subset = "test_i", )
    generate_summarys(model, tokenized_test_ni, test_ni, ruta = f"{carpeta}/resums_llargs/resums_test_ni_original.json", subset = "test_ni")
    for token in tokens_fonts:
        generate_summarys(model, tokenized_test_i, test_i, ruta = f"{carpeta}/resums_llargs/resums_test_i_forçat_{token}.json", forced_token = token, subset = "test_i")
        generate_summarys(model, tokenized_test_ni, test_ni,  ruta = f"{carpeta}/resums_llargs/resums_test_ni_forçat_{token}.json", forced_token = token, subset = "test_ni")
    ...
    generate_summary_font_correcta(model, test_i, ruta = f"{carpeta}/resums_llargs/resums_test_i_font_correcta.json")
