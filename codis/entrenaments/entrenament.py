...
validacio = datasets.load_dataset("json", data_files="../corpus_anonimitzat/" + llengua +"/validation.json.gz")
entrenament = datasets.load_dataset("json", data_files="../corpus_anonimitzat/" + llengua +"/train.json.gz")
...
checkpoint = "../pre-nasca" #agafem com a model previ el que no ha aplicat nasca encara
...
"""#Preparem una primera avaluació amb el dataset i el model originals per veure en quina situació ens trobem 
eval_dataloader = DataLoader(ds_combinat_tokenized["validation"], batch_size=8, collate_fn=data_collator)
device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
model.to(device)

metric = evaluate.load("rouge")
model.eval()
#Avaluem el model original amb el dataset nou
for batch in eval_dataloader:
    ...
    #és precís que primer ho passem a numpy per tal de poder fer el where
    referencia_memoria = batch["labels"].detach().cpu().numpy()
    resum_referencia = np.where(referencia_memoria != -100, referencia_memoria, tokenizer.pad_token_id)
    #els descodifiquem per tal que no siguen tokens, sino que siga text comparable.
    decoded_resum_referencia = tokenizer.batch_decode(resum_referencia, skip_special_tokens=True)
    decoded_resum_generat = tokenizer.batch_decode(predictions, skip_special_tokens=True)
    #rouge espera que els resums estiguen separats per un \n
    decoded_resum_generat = ["\n".join(nltk.sent_tokenize(pred.strip(), language="spanish")) for pred in decoded_resum_generat]
    decoded_resum_referencia = ["\n".join(nltk.sent_tokenize(label.strip(), language="spanish")) for label in decoded_resum_referencia]
    metric.add_batch(predictions=decoded_resum_generat, references=decoded_resum_referencia)
resultat = metric.compute()
print(resultat)
# Resetegem el model perquè voliem veure els resultats però que esta avaluació no afecte a l'entrenament
